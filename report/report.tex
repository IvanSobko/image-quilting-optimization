...
The above pseudocode analysis tells us that (1) the output overlap region is read multiple times and (2) the input texture is read multiple times. In an ideal world, both the output overlap region and input texture should fit into cache. Similar to the asymptotic analysis, for the purpose of the cache miss analysis, assume that the input image is square and has width $I_w$ pixels. Further assume that the blocks are square and that their width and height are linearly proportional to the dimensions of the input image. Then we have that the block height and width are given by $B_w = b I_w$ pixels (for some valid $0 < b < 1$ with $b \in \R$).

In the worst and most common case we have a corner overlap, and because the overlap size is 1/6 of the block size, we know the overlap region contains
\[
  \frac{1}{6} B_w ^2 + \frac{1}{6} B_w ^2 - \frac{1}{6}^2 B_w ^2 = \frac{11}{36} B_w ^2 \; \text{pixels}.
\]
Recall that each pixel consists of 4 bytes. To avoid capacity misses, the output overlap region and input texture must fit into cache, meaning the cache must fit
\[
  \frac{11}{9} b ^2 I_w ^2 + 4 I_w^2 \; \text{bytes}
\]
To get a practical sense of this quantity, let's use our common block width multiplier of $b = \frac{1}{4}$ and suppose the input texture is large (with respect to the baseline implementation runtime), say $I_w = 2^{16}$. Then, the output overlap region and input texture take consist of
\[
  \frac{11}{9*16} (2^{16}) ^2 + 4(2^{16}) ^2 \approx 1.75 e 7 \text{KiB}
\]


By reordering the inner and outer two loops, we can iterate over the pixels of the overlap region exactly once:
