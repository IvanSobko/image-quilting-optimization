...
namespace
{
    ...
    float computeErrorSum( ImageQuilting* imgQuilt,
        ...
        int width, int height)
    {
            ...
            ++ix, ++ixA, ++ixB)
        {
                ...
                ++iy, ++iyA, ++iyB)
            {
                ...
                imgQuilt->setFlopCount(imgQuilt->getFlopCount() + ImageQuilting::SQRT_NB_FLOPS + 1);
            }
        }
        return sqrErrSum;
    }

    float computeMSE( ImageQuilting* imgQuilt,
        ...
        int overlapH)
    {
        ...
        if(dstX > 0 && dstY <= 0)
        {
            const float sqrErrSum = computeErrorSum( imgQuilt,
                src, srcX, srcY, dst, dstX, dstY, overlapW, blockH);

            const int pixelCnt = overlapW * blockH;

            imgQuilt->setFlopCount(imgQuilt->getFlopCount() + ImageQuilting::DIV_NB_FLOPS + 1);
        ...
        const float A = computeErrorSum( imgQuilt,
            src, srcX, srcY, dst, dstX, dstY, overlapW, overlapH);

        const float B = computeErrorSum( imgQuilt,
            src, srcX, srcY + overlapH, dst, dstX, dstY + overlapH,

            overlapW, blockH - overlapH);
        const float C = computeErrorSum( imgQuilt,
        ...
        imgQuilt->setFlopCount(imgQuilt->getFlopCount() + ImageQuilting::DIV_NB_FLOPS + 8);
    ...
    void computeVerticalCutCost( ImageQuilting* imgQuilt,
        ...
        Image<MinCostCutUnit> &result)
    {
        ...
        for(int xi = 0; xi < width - 1; ++xi) {
            ...
            imgQuilt->setFlopCount(imgQuilt->getFlopCount() + ImageQuilting::SQRT_NB_FLOPS);
        }
        ...
        for(int yi = 1; yi < height; ++yi)
        {
            for(int xi = 0; xi < width - 1; ++xi)
            {
                ...
                imgQuilt->setFlopCount(imgQuilt->getFlopCount() + 4);
            }
        }
    }

    void computeHorizontalCutCost( ImageQuilting* imgQuilt,
        const Image<Float3>     &A,
        const ImageView<Float3> &B,
        int xA,    int yA,
        int xB,    int yB,
     ...
     */
    std::vector<int> computeVerticalMinCostCut( ImageQuilting* imgQuilt,
        const Image<Float3>     &A,
        const ImageView<Float3> &B,
        int xA,    int yA,
        int xB,    int yB,
        int width, int height)
    {
        ...
        computeVerticalCutCost( imgQuilt,
        ...
        for(int yi = height - 2; yi >= 0; --yi)
        {
            ...
            imgQuilt->setFlopCount(imgQuilt->getFlopCount() + 1);
        }
     ...
     */
    std::vector<int> computeHorizontalMinCostCut( ImageQuilting* imgQuilt,
        const Image<Float3>     &A,
        const ImageView<Float3> &B,
        int xA,    int yA,
        int xB,    int yB,
        int width, int height)
    {
        ...
        computeHorizontalCutCost( imgQuilt,
            A, B, xA, yA, xB, yB, width, height, costs);
    ...
    int                  generatedHeight)
{
    ...
    for(int blockY = 0; blockY < blockCountY; ++blockY)
    {
        ...
        for(int blockX = 0; blockX < blockCountX; ++blockX)
        {
            ...
            this->setFlopCount(this->getFlopCount() + 2);
    ...
    std::default_random_engine  &rng)
{
    ...
    for(int srcY = 0; srcY < src.height() - blockH_; ++srcY)
    {
        ...
        {
            const float mse = computeMSE( this,
    ...
    int                      y)
{
    ...
    if(x > 0 && y == 0)
    {
        const auto verticalCut = computeVerticalMinCostCut( this,
    ...
    {
        const auto horizontalCut = computeHorizontalMinCostCut( this,
...
void ImageQuilting::setFlopCount(int64_t flops) {
    this->flopCount = flops;
}
